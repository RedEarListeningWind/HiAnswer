 ...
 *     (3)快速双击主按钮会报错 2020.07.07 赤耳听风 已经解决
 */
...
var InputBox = floaty.rawWindow(
    <vertical id="inputW" w="*" h="auto"   bg="#ccffffff" alpha="0">
        
        <input id="input" w="*" h="auto" hint="请输入"  size="15sp" line="3" textColorHint="#2196f3"/>
        <horizontal>
            <button id="Copy" w="50" h="40"style="Widget.AppCompat.Button.Colored" text="复制"/>
            <button id="Paste" w="50" h="40"style="Widget.AppCompat.Button.Colored" text="粘贴"/>
            <button id="SearchQuestions" w="50" h="40"style="Widget.AppCompat.Button.Colored" text="查题"/>
            <button id="Move" w="50" h="40"style="Widget.AppCompat.Button.Colored" text="移动"/>
        </horizontal>
    </vertical>

);
InputBox.setTouchable(false);
InputBox.setPosition(device.width / 5, device.height/8);
InputBox.setSize(device.width * 3 / 5, -2);
//************************
 ...
 */
//******************
var Overall_Search_Engine = null;
//var Overall_Search_Engine_isNormal = false;

var Search_Questions_Engine = null;
//var Search_Questions_Engine_isNormal = false;

var Show_Answer_Engine = null;
//var Show_Answer_Engine_isNormal = false;
    ...
    <frame >//子菜单悬浮窗
        <frame id="id_logo" w="150" h="210" alpha="0"  >
            <frame id="id_0" w="44" h="44" margin="33 0 0 0" alpha="1">
                ...
                <img w="28" h="28" src="@drawable/ic_border_color_black_48dp" tint="#ffffff" gravity="center" layout_gravity="center" />
...
var win_1 = floaty.rawWindow(
    <frame id="logo" w="44" h="44" alpha="0.4" >//悬浮按钮
        ...
        <img id="img_logo" w="32" h="32" src="file://img/Icon.jpg" gravity="center" layout_gravity="center" scaleType="fitCenter" circle="true"/>
...
var win_2 = floaty.rawWindow(
    <frame id="logo" w="{{device.width}}px" h="44" alpha="0" >//悬浮按钮 弹性替身
        <img w="44" h="44" src="#ffffff" circle="true" alpha="0.8" />
        <img id="img_logo" w="32" h="32" src="file://img/Icon.jpg" margin="6 6" scaleType="fitCenter" circle="true"/>
    </frame>
)
 ...
 * 子菜单点击事件
 */
//var SearchQuestions_click_Date = Date.now();
var Electorate_click_Date = Date.now();

function img_down() {
    InputBox.disableFocus();
    win_1.logo.attr("alpha", "0.4")
    logo_switch = false
    动画()
}
win.id_0_click.on("click", () => {
    if (InputBox.inputW.alpha == "0") {
        InputBox.setPosition(device.width / 5, win_1.getY());
        InputBox.setTouchable(true);
        InputBox.inputW.alpha = "4";
        toastLog("输入栏打开了");
    } else {
        InputBox.setTouchable(false);
        InputBox.inputW.alpha = "0";
        toastLog("输入栏关闭了");
    }
    img_down()
})
...
win.id_2_click.on("click", () => {
    ...
    if (Date.now() - Electorate_click_Date > 1000) {
        Electorate_click_Date = Date.now();
        toastLog(Date.now() - Electorate_click_Date);
        //Overall_Search();
        Overall_Search_Engine = implement_ScriptEngine("全局搜索.js", "implement_Overall_Search", "true");
    } else {
        toastLog("点击过于频繁");
    }
...
win.id_3_click.on("click", () => {
    if (Overall_Search_Engine != null) {
        Overall_Search_Engine.getEngine().emit("implement_Overall_Search", false);
    }
    if (Show_Answer_Engine != null) {
        Show_Answer_Engine.getEngine().emit("implement_Show_Answer", -1);
    }
    if (Search_Questions_Engine != null) {
        Search_Questions_Engine.getEngine().emit("implement_Search_Questions", -1, null);
    }
    /*
    for (var i = 0, engines_All = engines.all(); engines_All.length > i; i++) {
        if (engines_All[i].getSource().toString().indexOf("main.js") != -1) {
            engines_All[i].emit("Keys_Window_Engine", false);
        }
    }
   */
...
//InputBox.inputW["minWidth"] = "600";
InputBox.input.on("key", function(keyCode, event) {
    if (event.getAction() == KeyEvent.ACTION_UP && keyCode == keys.back) {
        InputBox.disableFocus();
        event.consumed = true;

    }
    setTimeout(inputSize(), 800);
});
InputBox.input.on("touch_down", () => {
    InputBox.requestFocus();
    InputBox.input.requestFocus();
});
InputBox.Copy.click(() => {
    setClip(InputBox.input.text());
    toastLog("已经复制题目");
});
InputBox.Paste.click(() => {
    ui.run(function() {
        InputBox.input.setText(getClip());

    });
});


var SearchQuestions_click_Date = new Date().getTime();
InputBox.SearchQuestions.click(() => {
    //addLogList("输入搜索：" + Keys_Window.input.text(), "#8d6e63");
    if (new Date().getTime() - SearchQuestions_click_Date > 1000) {
        if (Keys_Window.input.text().length > 3) {
            SearchQuestions_click_Date = new Date().getTime();

            toastLog(++implement_i);
            //Show_Answer();
            //Search_Questions(Keys_Window.input.text());
            Show_Answer_Engine = implement_ScriptEngine("显示答案.js", "implement_Show_Answer", "implement_i");
            //sleep(800);
            Search_Questions_Engine = implement_ScriptEngine("查题.js", "implement_Search_Questions", "implement_i,sText", Keys_Window.input.text());
...
var MdownTime = null;
InputBox.Move.setOnTouchListener(function(view, event) {
    switch (event.getAction()) {
        case event.ACTION_DOWN:
            x = event.getRawX();
            y = event.getRawY();
            windowX = InputBox.getX();
            windowY = InputBox.getY();
            MdownTime = new Date().getTime();
            return true;
        case event.ACTION_MOVE:
            //移动手指时调整悬浮窗位置
            InputBox.setPosition(windowX + (event.getRawX() - x),
                windowY + (event.getRawY() - y));
            //如果按下的时间超过2秒判断为长按，退出脚本
            if (new Date().getTime() - MdownTime > 2000) {
                if(new Date().getTime() - MdownTime < 5000){
        ...
        case event.ACTION_UP:
            ...
            if (Math.abs(event.getRawY() - y) < 5 && Math.abs(event.getRawX() - x) < 5) {
                if (InputBox.input.getVisibility() == 0) {
                    InputBox.setSize(-2, -2);
                    InputBox.input.setVisibility(8);
                    InputBox.Copy.setVisibility(8);
                    InputBox.Paste.setVisibility(8);
                    InputBox.SearchQuestions.setVisibility(8);
                } else if (InputBox.input.getVisibility() == 8) {
                    InputBox.setSize(device.width * 3 / 5, -2);
                    InputBox.input.setVisibility(0);
                    InputBox.Copy.setVisibility(0);
                    InputBox.Paste.setVisibility(0);
                    InputBox.SearchQuestions.setVisibility(0);
                }
            }
            return true;
    }
...
function inputSize(){
    console.error("1");
    if(InputBox.input.text().length <= 17){
        InputBox.setSize(-2,-2);
...
//监听Title事件  接收来自全局搜索的题目
events.on("Title", function(sText) {

    ui.run(function() {
        InputBox.input.setText(sText);
        //inputSize();

        if (InputBox.inputW.alpha != "4") {
            InputBox.setPosition(device.width / 5, win_1.getY());
            InputBox.setTouchable(true);
            InputBox.inputW.alpha = "4";
            setTimeout(() => {
    ...
    toastLog(++implement_i);
    //Show_Answer();
    //Search_Questions(sText);
    Show_Answer_Engine = implement_ScriptEngine("显示答案.js", "implement_Show_Answer", "implement_i");
    //sleep(800);
    Search_Questions_Engine = implement_ScriptEngine("查题.js", "implement_Search_Questions", "implement_i,sText", sText);
...
function implement_ScriptEngine(ScriptEngine_Name, ScriptEngine_eventName, Information, sText) {
    var ScriptEngineOne = null;
    if (ScriptEngine_eventName == "implement_Overall_Search") {
        ScriptEngineOne = Overall_Search_Engine;
    } else if (ScriptEngine_eventName == "implement_Show_Answer") {
        ScriptEngineOne = Show_Answer_Engine;
    } else if (ScriptEngine_eventName == "implement_Search_Questions") {
        ScriptEngineOne = Search_Questions_Engine;
    } else {
        toastLog("线程启动错误");
        engines.myEngine().forceStop();
    }
    //console.info(Information);
    //toastLog(eval(Information));
    if (!engine_isNormal(ScriptEngine_Name)) {

        ScriptEngineOne = engines.execScriptFile("./" + ScriptEngine_Name);

        setTimeout(() => {

            eval("ScriptEngineOne.getEngine().emit(ScriptEngine_eventName," + Information + ")");

        }, 300);

    } else {
        if (ScriptEngineOne == null) {
            for (var i = 0, engines_All = engines.all(); engines_All.length > i; i++) {
                if (engines_All[i].getSource().toString().indexOf(ScriptEngine_Name) != -1) {
                    //engines_All[i].emit("Normal", "Overall_Search");
                    ScriptEngineOne = engines_All[i];
                }
            }
        } else {
            //toastLog("刚好"+ScriptEngineOne.getEngine());
            eval("ScriptEngineOne.getEngine().emit(ScriptEngine_eventName," + Information + ")");
        }


        /*
        Overall_Search_Engine_isNormal = false;

        Overall_Search_Engine.getEngine().emit("implement_Overall_Search", true);

        setTimeout(() => {

            if (Overall_Search_Engine_isNormal == false) {
                log("全局搜索线程：" + false);
                Overall_Search_Engine = null;
                Overall_Search();
            }
    ...
    return ScriptEngineOne;
}
//判断脚本引擎是否存活
function engine_isNormal(ScriptEngine_Name) {
    var Normal = false;
    for (var i = 0, engines_All = engines.all(); engines_All.length > i; i++) {
        console.error(engines_All[i]);
